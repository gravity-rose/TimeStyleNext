...
static void update_clock_and_date_area_layer(Layer *l, GContext* ctx, FContext* fctx) {
  ...
  // adjust vertical position based on sidebar position
  int apptBarH = ApptBar_get_height();
  if(settings.sidebarLocation == BOTTOM) {
    v_adjust -= 3;
    if(apptBarH > 0) {
      v_adjust += apptBarH;
      font_size = font_size * (fullscreen_bounds.size.h - apptBarH) / fullscreen_bounds.size.h;
    }
  } else {
    ...
    v_adjust += SidebarWidgets_fixedWidgetHeight - obstruction_height - 3;
    if(apptBarH > 0) {
      font_size = font_size * (fullscreen_bounds.size.h - apptBarH) / fullscreen_bounds.size.h;
    }
  }
...
#ifndef PBL_COLOR
  if(settings.timeColor.argb == GColorLightGrayARGB8 && settings.timeBgColor.argb == GColorWhiteARGB8) {
  ...
  if(settings.centerTime == false || settings.clockFontId == FONT_SETTING_BOLD_H || settings.clockFontId == FONT_SETTING_BOLD_M) {
    // draw hours
    ...
    strncpy(time, time_date_hours, sizeof(time));
    strncat(time, ":" , sizeof(time) - strlen(time) - 1);
    strncat(time, time_date_minutes, sizeof(time) - strlen(time) - 1);
  ...
  strncpy(time_date_currentDate, settings.languageDayNames[time_date_currentDayName], sizeof(time_date_currentDate));
  strncat(time_date_currentDate, " " , sizeof(time_date_currentDate) - strlen(time_date_currentDate) - 1);
  strncat(time_date_currentDate, time_date_currentDayNum, sizeof(time_date_currentDate) - strlen(time_date_currentDate) - 1);
  strncat(time_date_currentDate, " " , sizeof(time_date_currentDate) - strlen(time_date_currentDate) - 1);
  strncat(time_date_currentDate, settings.languageMonthNames[time_date_currentMonth], sizeof(time_date_currentDate) - strlen(time_date_currentDate) - 1);

  // draw date
  int dateY = fullscreen_bounds.size.h / 2 - 11 + v_adjust;
  if(apptBarH > 0) {
    dateY -= apptBarH / 2;
  }
  ...
  graphics_draw_text(ctx,
                     ...
                     date_font,
                     GRect(0, dateY, fullscreen_bounds.size.w, 30),
                     GTextOverflowModeFill,
...
void ClockArea_update_fonts(void) {
  ...
  if(prev_clockFontId != settings.clockFontId) {
    ...
    prev_clockFontId = settings.clockFontId;
  }
}
